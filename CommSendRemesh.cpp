...
void CommSendRemesh(Home_t *home) {
  ...
  for (idst = 0; idst < home->remoteDomainCount; idst++) {
    ...
    MPI_Isend(&outMsgLen, 1, MPI_INT, domainIdx, MSG_REMESH_LEN, MPI_COMM_WORLD,
              &home->outRequests[idst]);
  } /* end for (idst = 0; ...) */
  ...
  for (isrc = 0; isrc < home->remoteDomainCount; isrc++) {
    ...
    MPI_Irecv(remDom->inBuf, remDom->inBufLen, MPI_BYTE, domainIdx, MSG_REMESH,
              MPI_COMM_WORLD, &home->inRequests[isrc]);
  }
  ...
  for (idst = 0; idst < home->remoteDomainCount; idst++) {
    ...
    MPI_Isend(home->opList, outMsgLen, MPI_BYTE, domainIdx, MSG_REMESH,
