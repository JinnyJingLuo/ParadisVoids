...
static void AddSecondaryGhostRequest(Home_t *home, Node_t *node, int segID,
  ...
  // the line above was altered by Ahmed M. Hussein on June 1st 2014.
  // remDomID = node->nbrTag[segID].domainID;
...
static void Pack1SecondaryGhost(Node_t *node, real8 **outBuf, int *outBufSize,
  ...
  buf[bufOffset++] = node->dNx;
  buf[bufOffset++] = node->dNy;
  buf[bufOffset++] = node->dNz;

  buf[bufOffset++] = node->oldx;
  buf[bufOffset++] = node->oldy;
  buf[bufOffset++] = node->oldz;
  ...
  for (i = 0; i < node->numNbrs; i++) {
    ...
    buf[bufOffset++] = (real8)node->piChainID[i];
  }
...
static void PackSecondaryGhostResponse(Home_t *home, int *inBuf, int inBufSize,
  ...
  *outBufSize = 1;
  ...
  for (i = 0; i < numRequestedNodes; i++) {
    ...
    if (node == NULL) {
      printf("PackSecondaryGhostResponse(): Task %d can't find (%d,%d)\n",
...
static void UnpackSecondaryGhostResponse(Home_t *home, real8 *inBuf) {
  ...
  for (i = 0; i < numNodes; i++) {
    ...
    if (tmpNode != (Node_t *)NULL) {
      ...
      nodeVals = FLTS_PER_GHOST2_NODE + FLTS_PER_GHOST_ARM * numArms;
      bufOffset += nodeVals;
      continue;
    }
    ...
    node->dNx = inBuf[bufOffset++];
    node->dNy = inBuf[bufOffset++];
    node->dNz = inBuf[bufOffset++];

    node->oldx = inBuf[bufOffset++];
    node->oldy = inBuf[bufOffset++];
    node->oldz = inBuf[bufOffset++];
    ...
    for (armID = 0; armID < node->numNbrs; armID++) {
      ...
      node->piChainID[armID] = (int)inBuf[bufOffset++];
    }
    ...
    if (remDom == (RemoteDomain_t *)NULL) {
      ...
      totRemDomCount =
          home->remoteDomainCount + home->secondaryRemoteDomainCount;

      home->remoteDomains =
          (int *)realloc(home->remoteDomains, totRemDomCount * sizeof(int));
    ...
    if (node->myTag.index >= remDom->maxTagIndex) {
      remDom->nodeKeys = (Node_t **)realloc(
          remDom->nodeKeys, (node->myTag.index + 1) * sizeof(Node_t *));
...
void CommSendSecondaryGhosts(Home_t *home) {
  ...
  numSendReqBufs = 0;
  numRemoteDomains = home->remoteDomainCount + home->secondaryRemoteDomainCount;
  ...
  MPI_Allreduce(localMsgCnts, globalMsgCnts, home->numDomains, MPI_INT, MPI_SUM,
                MPI_COMM_WORLD);
  ...
  for (i = 0; i < numRecvReqBufs; i++) {
    MPI_Irecv(&recvReqBufLen[i], 1, MPI_INT, MPI_ANY_SOURCE, MSG_GHOST2_REQ_LEN,
  ...
  for (i = 0; i < numSendReqBufs; i++) {
    ...
    MPI_Isend(&remDom->outBufLen, 1, MPI_INT, reqDomList[i], MSG_GHOST2_REQ_LEN,
              MPI_COMM_WORLD, &home->outRequests[i]);
  }
  ...
  for (i = 0; i < numRecvReqBufs; i++) {
    ...
    MPI_Irecv(remDom->inBuf, recvReqBufLen[i], MPI_INT, respDomList[i],
              MSG_GHOST2_REQ, MPI_COMM_WORLD, &home->inRequests[i]);
  }
  ...
  for (i = 0; i < numSendReqBufs; i++) {
    ...
    remDom = home->remoteDomainKeys[remDomID];
    MPI_Isend(remDom->outBuf, remDom->outBufLen, MPI_INT, remDomID,
              MSG_GHOST2_REQ, MPI_COMM_WORLD, &home->outRequests[i]);
  }
  ...
  for (i = 0; i < numRecvReqBufs; i++) {
    ...
    remDom = home->remoteDomainKeys[remDomID];
    PackSecondaryGhostResponse(home, (int *)remDom->inBuf, remDom->inBufLen,
                               (real8 **)&remDom->outBuf, &remDom->outBufLen);
    free(remDom->inBuf);
  ...
  for (i = 0; i < numRecvRespBufs; i++) {
    remDomID = reqDomList[i];
    remDom = home->remoteDomainKeys[remDomID];
    MPI_Irecv(&remDom->inBufLen, 1, MPI_INT, remDomID, MSG_GHOST2_RESPONSE_LEN,
              MPI_COMM_WORLD, &home->inRequests[i]);
  }
  ...
  for (i = 0; i < numSendRespBufs; i++) {
    ...
    remDom = home->remoteDomainKeys[remDomID];
    MPI_Isend(&remDom->outBufLen, 1, MPI_INT, remDomID, MSG_GHOST2_RESPONSE_LEN,
              MPI_COMM_WORLD, &home->outRequests[i]);
  }
  ...
  for (i = 0; i < numRecvRespBufs; i++) {
    ...
    MPI_Irecv(remDom->inBuf, remDom->inBufLen, MPI_DOUBLE, remDomID,
              MSG_GHOST2_RESPONSE, MPI_COMM_WORLD, &home->inRequests[i]);
  }
  ...
  for (i = 0; i < numSendRespBufs; i++) {
    remDomID = respDomList[i];
    remDom = home->remoteDomainKeys[remDomID];
    MPI_Isend(remDom->outBuf, remDom->outBufLen, MPI_DOUBLE, respDomList[i],
              MSG_GHOST2_RESPONSE, MPI_COMM_WORLD, &home->outRequests[i]);
  }
   ...
   */
  for (i = 0; i < numRecvRespBufs; i++) {
    MPI_Waitany(numRecvRespBufs, home->inRequests, &recvIndex, home->inStatus);
