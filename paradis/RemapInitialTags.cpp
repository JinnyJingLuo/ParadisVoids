...
static int TagMapCompare(const void *a, const void *b) {
  ...
  if (tagMap1->oldTag.domainID < tagMap2->oldTag.domainID)
    return (-1);
  if (tagMap1->oldTag.domainID > tagMap2->oldTag.domainID)
    return (1);

  if (tagMap1->oldTag.index < tagMap2->oldTag.index)
    return (-1);
  if (tagMap1->oldTag.index > tagMap2->oldTag.index)
    return (1);
...
void DistributeTagMaps(Home_t *home) {
  ...
  for (i = 0; i < home->remoteDomainCount; i++) {
    ...
    MPI_Irecv(&remDom->inBufLen, 1, MPI_INT, remDomIndex, MSG_TAGREMAP_LEN,
  ...
  for (i = 0; i < home->remoteDomainCount; i++) {
    remDomIndex = home->remoteDomains[i];
    remDom = home->remoteDomainKeys[remDomIndex];
    MPI_Isend(&tagBufLen, 1, MPI_INT, remDomIndex, MSG_TAGREMAP_LEN,
  ...
  for (i = 0; i < home->remoteDomainCount; i++) {
    ...
    MPI_Irecv(remDom->inBuf, remDom->inBufLen / sizeof(int), MPI_INT,
              remDomIndex, MSG_TAGREMAP, MPI_COMM_WORLD, &home->inRequests[i]);
  }
  /*
   ...
   */
  for (i = 0; i < home->remoteDomainCount; i++) {
    MPI_Waitany(home->remoteDomainCount, home->inRequests, &reqIndex,
                &reqStatus);
  ...
  if (home->tagMapEnts > 0) {
    qsort(home->tagMap, home->tagMapEnts, sizeof(TagMap_t), TagMapCompare);
