...
void WriteDensityField(Home_t *home, char *fileName) {
  ...
  if ((nx > MAXDENSITYMESH) || (ny > MAXDENSITYMESH) || (nz > MAXDENSITYMESH)) {
    if (thisDomain == 0) {
  ...
  xmax = 0.5 * param->Dimensions[X];
  ymin = -0.5 * param->Dimensions[Y];
  ymax = 0.5 * param->Dimensions[Y];
  zmin = -0.5 * param->Dimensions[Z];
  zmax = 0.5 * param->Dimensions[Z];
  ...
  if (thisDomain == 0) {
     ...
     */
    fprintf(fp, "ParaDiS dislocation density file "
                "(viewable by vaspview)\n");
    fprintf(fp, "1.0\n");
    fprintf(fp, "%e 0 0\n", 1.0);
    fprintf(fp, "0 %e 0\n", Ly / Lx);
    fprintf(fp, "0 0 %e\n", Lz / Lx);
  ...
  totDensity = (real8(*)[MAXDENSITYMESH][MAXDENSITYMESH][MAXDENSITYMESH])calloc(
      1, sizeof(real8) * elemCount);

#ifdef PARALLEL
  MPI_Reduce(&den[0][0][0], totDensity, elemCount, MPI_DOUBLE, MPI_SUM, 0,
             MPI_COMM_WORLD);
#else
  for (i = 0; i < nx; i++) {
    for (j = 0; j < ny; j++) {
