  ...
  for (idom = 0; idom < home->remoteDomainCount; idom++) {
    ...
    for (opidx = 0; opidx < rcvOpCount; opidx++) {
      ...
      case INSERT_ARM:
        ...
        InsertArm(home, node1, &tag2, op->bx, op->by, op->bz, op->nx, op->ny,
                  op->nz, op->dom3, 0);
        break;
 ...
 */
        ...
        /*
         *                              Insure the node is assigned to the
         * proper cell
         */
        ...
        cellIndex[X] =
            (int)(node2->x + 0.5 * param->Dimensions[X]) / (int)cellSize[X];
        cellIndex[Y] =
            (int)(node2->y + 0.5 * param->Dimensions[Y]) / (int)cellSize[Y];
        cellIndex[Z] =
            (int)(node2->z + 0.5 * param->Dimensions[Z]) / (int)cellSize[Z];
        ...
        node2->cellIdx =
            EncodeCellIdx(home, cellIndex[X], cellIndex[Y], cellIndex[Z]);
        break;

      /*
       *			Reset the forces on the specified segment of
       *node1. The op structure x,y,z values are loaded here with force
       *values.
       */
      ...
      case (CHANGE_ARM_BURG):
        ...
        ChangeArmBurg(home, node1, &tag2, op->bx, op->by, op->bz, op->nx,
                      op->ny, op->nz, 0, DEL_SEG_NONE, bRemoveArm);
        break;
      ...
      case RESET_GLIDE_PLANE:
        ...
        ResetGlidePlane(home, newPlane, &tag1, &tag2, 0);
        break;
      ...
      case CHANGE_NODE_TYPE:
        ...
        if (node1 == NULL)
          break;
        ChangeNodeType(home, node1, op->dom2, 0);
        break;
