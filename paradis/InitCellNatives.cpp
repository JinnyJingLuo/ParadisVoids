...
void InitCellNatives(Home_t *home) {
   ...
   */
  for (cIndex = 1; cIndex <= param->nXcells; cIndex++) {
    cellMin = rint((-0.5 * param->Dimensions[X]) + (cIndex - 1) * xCellSize);
    cellMax = rint((-0.5 * param->Dimensions[X]) + (cIndex * xCellSize));

    if ((xMin >= cellMin) && (xMin < cellMax))
      xCellMin = cIndex;
    if ((xMax <= cellMax) && (xMax > cellMin))
      xCellMax = cIndex;
  }
   ...
   */
  for (cIndex = 1; cIndex <= param->nYcells; cIndex++) {
    cellMin = rint((-0.5 * param->Dimensions[Y]) + (cIndex - 1) * yCellSize);
    cellMax = rint((-0.5 * param->Dimensions[Y]) + (cIndex * yCellSize));

    if ((yMin >= cellMin) && (yMin < cellMax))
      yCellMin = cIndex;
    if ((yMax <= cellMax) && (yMax > cellMin))
      yCellMax = cIndex;
  }
   ...
   */
  for (cIndex = 1; cIndex <= param->nZcells; cIndex++) {
    cellMin = rint((-0.5 * param->Dimensions[Z]) + (cIndex - 1) * zCellSize);
    cellMax = rint((-0.5 * param->Dimensions[Z]) + (cIndex * zCellSize));

    if ((zMin >= cellMin) && (zMin < cellMax))
      zCellMin = cIndex;
    if ((zMax <= cellMax) && (zMax > cellMin))
      zCellMax = cIndex;
  }
  ...
  if (xCellMax > param->nXcells) {
    xCellMax = param->nXcells;
  }
  if (yCellMin < 1) {
    yCellMin = 1;
  }
  if (yCellMax > param->nYcells) {
    yCellMax = param->nYcells;
  }
  if (zCellMin < 1) {
    zCellMin = 1;
  }
  if (zCellMax > param->nZcells) {
    zCellMax = param->nZcells;
  }
  /*
  ...
  home->cellKeys = (Cell_t **)calloc(numCells, sizeof(Cell_t *));
   ...
   */
  home->cellList = (int *)calloc(numCells, sizeof(int));
  home->cellCount = 0;
  ...
  for (xIndex = xCellMin; xIndex <= xCellMax; xIndex++) {
    for (yIndex = yCellMin; yIndex <= yCellMax; yIndex++) {
      for (zIndex = zCellMin; zIndex <= zCellMax; zIndex++) {
         ...
         * that we don't have to repeatedly call DecodeCellIdx() later on.
         */
        pCell->xIndex = xIndex;
        pCell->yIndex = yIndex;
        pCell->zIndex = zIndex;
