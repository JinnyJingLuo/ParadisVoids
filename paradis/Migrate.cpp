...
static void BuildMigLists(Home_t *home, int *migCommList, int **migList,
  ...
  for (i = 0; i < nodeCount; i++) {
     ...
     */
    destDom = FindCoordDomain(home, node->x, node->y, node->z);
    ...
    migList[destDom] = (int *)realloc(migList[destDom],
                                      (migListCnt[destDom] + 1) * sizeof(int));
    ...
    if (j == sendBufCnt) {
      ...
      if (sendBufCnt > allocatedValues) {
        allocatedValues += 25;
        sendBufList =
            (int *)realloc(sendBufList, allocatedValues * sizeof(int));
      }
...
static void PackMigrators(Home_t *home, int remDomID, int *migList,
  ...
  numVals = migNodeCount * VALS_PER_MIG_NODE + armCount * VALS_PER_MIG_ARM +
            VALS_PER_MIG_EXTRA;
  ...
  for (i = 0; i < migNodeCount; i++) {
    ...
    buf[bufIndex++] = node->dNx;
    buf[bufIndex++] = node->dNy;
    buf[bufIndex++] = node->dNz;

    buf[bufIndex++] = node->oldx;
    buf[bufIndex++] = node->oldy;
    buf[bufIndex++] = node->oldz;
    ...
    for (j = 0; j < node->numNbrs; j++) {
      ...
      buf[bufIndex++] = (real8)node->piChainID[j];
    }
...
static void UnpackMigrators(Home_t *home, real8 *buf, int remDomID) {
  ...
  for (i = 0; i < nodeCount; i++) {
    ...
    node->dNx = buf[bufIndex++];
    node->dNy = buf[bufIndex++];
    node->dNz = buf[bufIndex++];

    node->oldx = buf[bufIndex++];
    node->oldy = buf[bufIndex++];
    node->oldz = buf[bufIndex++];
    ...
    for (j = 0; j < numNbrs; j++) {
      ...
      node->piChainID[j] = (int)buf[bufIndex++];
    }
  }
}
...
static void CommSendMigrators(Home_t *home, int *migCommList, int **migList,
  ...
  MPI_Allreduce(migCommList, glblMigCommList, numDomains, MPI_INT, MPI_SUM,
                MPI_COMM_WORLD);
  ...
  for (i = 0; i < numRecvBufs; i++) {
    MPI_Irecv(&recvBufLen[i], 1, MPI_INT, MPI_ANY_SOURCE, MSG_MIG_LENGTH,
   ...
   */
  for (i = 0; i < numSendBufs; i++) {
    MPI_Isend(&sendBufLen[i], 1, MPI_INT, sendBufDest[i], MSG_MIG_LENGTH,
  ...
  // Allocate receive buffers of the appropriate sizes and post the receives
  // associated with those buffers.
  for (i = 0; i < numRecvBufs; i++) {
    recvBuf[i] = (char *)malloc(recvBufLen[i]);
    numValues = recvBufLen[i] / sizeof(real8);
    MPI_Irecv(recvBuf[i], numValues, MPI_DOUBLE, recvStatus[i].MPI_SOURCE,
              MSG_MIG_NODES, MPI_COMM_WORLD, &recvRequest[i]);
  }

  // Send all the migrating nodes to the appropriate remote domains.
  for (i = 0; i < numSendBufs; i++) {
    numValues = sendBufLen[i] / sizeof(real8);
    MPI_Isend(sendBuf[i], numValues, MPI_DOUBLE, sendBufDest[i], MSG_MIG_NODES,
              MPI_COMM_WORLD, &sendRequest[i]);
  }
...
void Migrate(Home_t *home) {
   ...
   */
  BuildMigLists(home, migCommList, migList, migListCnt, &sendBufDest,
                &numSendBufs);
   ...
   *      other domains.
   */
  CommSendMigrators(home, migCommList, migList, migListCnt, sendBufDest,
                    numSendBufs);
