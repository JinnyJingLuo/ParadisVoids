...
void WriteRestart(Home_t *home, char *baseFileName, int ioGroup,
   ...
   *      simply be appended.
   */
  snprintf(ctrlFile, sizeof(ctrlFile), "%s/%s", DIR_RESTART, baseFileName);
  ...
  if (param->numIOGroups == 1) {
    snprintf(dataFile, sizeof(dataFile), "%s/%s%s", DIR_RESTART, fileName,
             NODEDATA_FILE_SUFFIX);
  } else {
    snprintf(dataFile, sizeof(dataFile), "%s/%s%s.%d", DIR_RESTART, fileName,
             NODEDATA_FILE_SUFFIX, ioGroup);
  }
  ...
  if (firstInGroup) {
    ...
    if (writePrologue) {
      ...
      if ((fpCtrl = fopen(ctrlFile, "w")) == (FILE *)NULL) {
        Fatal("WriteRestart: Open error %d on %s", errno, ctrlFile);
      }

      /*
       *              Persist the current precipitate structure to a restart-
       *              specific file and point the restart control file to it.
       *              This allows nucleation-mode runs to continue from the
       *              evolved precipitate state.
       */
      if (home->poPrecipitateServer != (ParadisPrecipitateServer *)NULL) {
        char precipFile[256];
        snprintf(precipFile, sizeof(precipFile), "%s/%s.precip", DIR_RESTART,
                 fileName);
        home->poPrecipitateServer->WritePrecipitateStructureSnapshot(
            std::string(precipFile));
        snprintf(param->PrecipitatesFileName, sizeof(param->PrecipitatesFileName),
                 "%s", precipFile);
      }
  ...
  for (i = 0; i < newNodeKeyPtr; i++) {
    ...
    if ((node->numNbrs == 1) && (node->constraint == UNCONSTRAINED)) {
      PrintNode(node);
      Fatal("WriteRestart: Node (%d,%d) singly linked!", node->myTag.domainID,
            node->myTag.index);
    }
    if (home->param->EnableTwinPlaneCrossSlip == 1) {
      fprintf(fp, "%d,%d\t\t%25.20f\t\t%25.20f\t\t%25.20f\t\t%d\t\t%d\t\t%d\n",
              node->myTag.domainID, node->myTag.index, node->x, node->y,
              node->z, node->numNbrs, node->constraint, node->csState);
    } else
      fprintf(fp, "%d,%d\t\t%25.20f\t\t%25.20f\t\t%25.20f\t\t%d\t\t%d\n",
              node->myTag.domainID, node->myTag.index, node->x, node->y,
              node->z, node->numNbrs, node->constraint);
    // write the surface normal
    fprintf(fp, "\t\t\t\t%25.20f\t\t%25.20f\t\t%25.20f\n", node->dNx, node->dNy,
            node->dNz);

    // Write the segment specific data
    for (iArm = 0; iArm < node->numNbrs; iArm++) {
      fprintf(fp, "\t\t%d,%d\t\t%25.20f\t\t%25.20f\t\t%25.20f\n",
              node->nbrTag[iArm].domainID, node->nbrTag[iArm].index,
              node->burgX[iArm], node->burgY[iArm], node->burgZ[iArm]);
      fprintf(fp, "\t\t\t\t%25.20f\t\t%25.20f\t\t%25.20f\t\t%d\n",
