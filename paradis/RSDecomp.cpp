...
void GetRSDecompCellDomainList(Home_t *home, int cellID, int *domCount,
   ...
   */
  xCellSize = param->Dimensions[X] / param->nXcells;
  yCellSize = param->Dimensions[Y] / param->nYcells;
  zCellSize = param->Dimensions[Z] / param->nZcells;

  xCellMin = rint((-0.5 * param->Dimensions[X]) + ((xCell - 1) * xCellSize));
  yCellMin = rint((-0.5 * param->Dimensions[Y]) + ((yCell - 1) * yCellSize));
  zCellMin = rint((-0.5 * param->Dimensions[Z]) + ((zCell - 1) * zCellSize));

  xCellMax = rint((-0.5 * param->Dimensions[X]) + (xCell * xCellSize));
  yCellMax = rint((-0.5 * param->Dimensions[Y]) + (yCell * yCellSize));
  zCellMax = rint((-0.5 * param->Dimensions[Z]) + (zCell * zCellSize));
  ...
  for (xDom = 0; xDom < param->nXdoms; xDom++) {
    if (domBoundX[xDom] > (xCellMax) || domBoundX[xDom + 1] < (xCellMin)) {
      continue;
    }
    ...
    for (yDom = 0; yDom < param->nYdoms; yDom++) {
      ...
      for (zDom = 0; zDom < param->nZdoms; zDom++) {
        ...
        if (numDomains >= maxDomains) {
          maxDomains += DOMAIN_CNT_INCREMENT;
          domains = (int *)realloc(domains, maxDomains * sizeof(int));
        }
      }
    }
  }
...
void ReadRSDecompBounds(void **filePtr, int numXDoms, int numYDoms,
  ...
  for (i = 0; i < numXDoms; i++) {
    ...
    if (saveDecomp)
      sscanf(inLine, "%lf", &xBnd[i]);
    for (j = 0; j < numYDoms; j++) {
      Getline(inLine, sizeof(inLine), fp);
      if (saveDecomp)
        sscanf(inLine, "%lf", &yBnd[i][j]);
      for (k = 0; k < numZDoms; k++) {
        Getline(inLine, sizeof(inLine), fp);
        if (saveDecomp)
          sscanf(inLine, "%lf", &zBnd[i][j][k]);
      }
      Getline(inLine, sizeof(inLine), fp);
      if (saveDecomp)
        sscanf(inLine, "%lf", &zBnd[i][j][k]);
...
void ReadBinRSDecompBounds(void *filePtr, int numXDoms, int numYDoms,
   ...
   */
  numValues =
      numXDoms * (numYDoms * (numZDoms + 1) + (numYDoms + 1)) + (numXDoms + 1);
 ...
 *
 *-------------------------------------------------------------------------*/
int FindRSDecompCoordDomain(Home_t *home, RSDecomp_t *decomp, real8 x, real8 y,
                            real8 z) {
...
static void DLBnewBounds(real8 *Bold, real8 *Bnew, real8 *times, int numParts,
  ...
  for (i = 0; i < (numParts - 1); i++) {
     ...
     */
    minBound =
        Bold[i + 1] - MIN(shiftLimit, (MAX_SHIFT * (Bold[i + 1] - Bold[i])));
    maxBound = Bold[i + 1] +
    ...
    for (j = 0; j < numParts + 1; j++) {
      if (Bnew[i] > Bold[j]) {
        ...
        wTotal +=
            times[j - 1] * ((Bold[j] - Bnew[i]) / (Bold[j] - Bold[j - 1]));
      }
      break;
    }
    ...
    if (Bnew[i] < Bold[i]) {
      ...
      if (wTarget < times[i]) // If it does not overlap the i+1 partition...
      {
        Bnew[i + 1] = Bold[i] + (wTarget / wDensity[i]);
      } else // Does overlap the i+1 partition...
      {
        wTarget -= times[i];
     ...
     */
      ...
      if (wRemaining >= wTarget) // Does not overlap previous i+1 partition
      {
        Bnew[i + 1] = Bnew[i] + (wTarget / wDensity[i]);
      } else // Overlaps previous i+1 partition
      {
        wTarget -= wRemaining;
        Bnew[i + 1] = Bold[i + 1] + (wTarget / wDensity[i + 1]);
      }
...
void DLBalanceY(Home_t *home, real8 *loadData) {
  ...
  for (i = 0; i < nXdoms; i++) {
    DLBnewBounds(decomp->domBoundY[i], Bnew, valY[i], nYdoms, shiftLimit);
...
void DLBalanceZ(Home_t *home, real8 *loadData) {
  ...
  for (i = 0; i < nXdoms; i++) {
    for (j = 0; j < nYdoms; j++) {
      DLBnewBounds(decomp->domBoundZ[i][j], Bnew, valZ[i][j], nZdoms,
                   shiftLimit);
...
void UniformRSDecomp(Param_t *param, RSDecomp_t **uniDecomp) {
  ...
  xCut = (real8 *)malloc((param->nXdoms + 1) * sizeof(real8));
  yCut = (real8 *)malloc((param->nYdoms + 1) * sizeof(real8));
  zCut = (real8 *)malloc((param->nZdoms + 1) * sizeof(real8));

  for (i = 0; i < param->nXdoms; i++) {
    xCut[i] = rint(-0.5 * param->Dimensions[X] + i * xDomLen);
  }
  xCut[param->nXdoms] = 0.5 * param->Dimensions[X];

  for (i = 0; i < param->nYdoms; i++) {
    yCut[i] = rint(-0.5 * param->Dimensions[Y] + i * yDomLen);
  }
  yCut[param->nYdoms] = 0.5 * param->Dimensions[Y];

  for (i = 0; i < param->nZdoms; i++) {
    zCut[i] = rint(-0.5 * param->Dimensions[Z] + i * zDomLen);
  }
  zCut[param->nZdoms] = 0.5 * param->Dimensions[Z];

  /*
   *      Create a new domain decomposition.
   */
  newXBounds = (real8 *)malloc((param->nXdoms + 1) * sizeof(real8));
  newYBounds = (real8 **)malloc(param->nXdoms * sizeof(real8 *));
  newZBounds = (real8 ***)malloc(param->nXdoms * sizeof(real8 **));
  ...
  for (xDom = 0; xDom < nXdoms; xDom++) {
    ...
    for (yDom = 0; yDom < nYdoms; yDom++) {
      ...
      newZBounds[xDom][yDom] = (real8 *)malloc((nZdoms + 1) * sizeof(real8));
...
void BroadcastRSDecomp(Home_t *home, RSDecomp_t *decomp) {
    ...
    for (xDom = 0; xDom < numXDoms; xDom++) {
      ...
      for (yDom = 0; yDom < numYDoms; yDom++) {
        ...
        for (zDom = 0; zDom < numZDoms; zDom++) {
          ...
          bcastBuf[bufIndex++] = decomp->domBoundZ[xDom][yDom][zDom];
        }
      }
    }
  }

  DSMPI::Broadcast((real8 *)bcastBuf, numVals, MPI_DOUBLE, 0);
  ...
  inDecomp->domBoundX = (real8 *)malloc((param->nXdoms + 1) * sizeof(real8));
  inDecomp->domBoundY = (real8 **)malloc(param->nXdoms * sizeof(real8 *));
  inDecomp->domBoundZ = (real8 ***)malloc(param->nXdoms * sizeof(real8 **));
  ...
  for (xDom = 0; xDom < numXDoms; xDom++) {
    ...
    inDecomp->domBoundY[xDom] = (real8 *)malloc((numYDoms + 1) * sizeof(real8));
    inDecomp->domBoundZ[xDom] = (real8 **)malloc(numYDoms * sizeof(real8 *));
    ...
    for (yDom = 0; yDom < numYDoms; yDom++) {
      ...
      for (zDom = 0; zDom < numZDoms; zDom++) {
        inDecomp->domBoundZ[xDom][yDom][zDom] = bcastBuf[bufIndex++];
      }

      inDecomp->domBoundZ[xDom][yDom][numZDoms] = 0.5 * param->Dimensions[Z];
    }

    inDecomp->domBoundY[xDom][numYDoms] = 0.5 * param->Dimensions[Y];
  }

  inDecomp->domBoundX[numXDoms] = 0.5 * param->Dimensions[X];
