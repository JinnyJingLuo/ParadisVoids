...
static void UnpackSegmentData(Home_t *home, real8 *buf) {
  ...
  for (i = 0; i < numSegs; i++) {
    ...
    if (tag1.domainID == thisDomain) {
      ...
      if (node == NULL) {
        printf("node (%d,%d) not found during segment force update\n",
...
void CommSendSegments(Home_t *home, int numRecvBufs, int numSendBufs,
  ...
  if (numSendBufs > 0) {
    PackSegmentData(home, numSendBufs, sendDomList, cellSegLists, cellSegCnts,
                    sendBuf, sendBufEnts);
  }
  /*
  ...
  for (i = 0; i < numRecvBufs; i++) {
    MPI_Irecv(&recvBufEnts[i], 1, MPI_INT, MPI_ANY_SOURCE, MSG_SEGDATA_LEN,
   ...
   */
  for (i = 0; i < numSendBufs; i++) {
    MPI_Isend(&sendBufEnts[i], 1, MPI_INT, sendDomList[i * 3], MSG_SEGDATA_LEN,
  ...
  for (i = 0; i < numRecvBufs; i++) {
    ...
    MPI_Irecv(recvBuf[i], recvBufEnts[i], MPI_DOUBLE,
              home->inStatus[i].MPI_SOURCE, MSG_SEGDATA, MPI_COMM_WORLD,
              &home->inRequests[i]);
  }
  /*
   ...
   */
  for (i = 0; i < numSendBufs; i++) {
    MPI_Isend(sendBuf[i], sendBufEnts[i], MPI_DOUBLE, sendDomList[i * 3],
              MSG_SEGDATA, MPI_COMM_WORLD, &home->outRequests[i]);
  }
  /*
