 ...
 *
 *-------------------------------------------------------------------------*/
void BindVar(ParamList_t *list, char *name, void *addr, int type, int cnt,
             int flags) {
  int pCnt;
  VarData_t *vList;
...
void WriteParam(ParamList_t *list, int index, FILE *fp) {
  ...
  for (i = minIndex; i < maxIndex; i++) {
     ...
     */
    if (type != V_COMMENT) {
      fprintf(fp, "%s = %s%s", list->varList[i].varName, lBracket, lineFeed);
    }

    for (j = 0; j < count; j++) {
      switch (type) {
      case V_INT:
        fprintf(fp, "  %d%s", ((int *)(list->varList[i].valList))[j], lineFeed);
        break;
      case V_DBL:
        fprintf(fp, "  %e%s", ((double *)(list->varList[i].valList))[j],
                lineFeed);
        break;
      case V_STRING:
        if (list->varList[i].valCnt == 1) {
          fprintf(fp, "  \"%s\"%s", (char *)list->varList[i].valList, lineFeed);
        } else {
          fprintf(fp, "  \"%s\"%s", ((char **)(list->varList[i].valList))[j],
                  lineFeed);
        }
        break;
      case V_COMMENT:
  ...
  for (itemCnt = 0, valCnt = 0;; valCnt++, itemCnt++) {
    ...
    case TOKEN_END_VAL_LIST:
      ...
      if (lBracketCnt < 0)
        return (0);
      if (lBracketCnt == 0) {
        if ((valsExpected == 0) || (valCnt == valsExpected)) {
          return (1);
        }
        if (valCnt < valsExpected)
          return (0);
      }
      valCnt--;
      break;
...
int LookupParam(ParamList_t *list, char *token) {
  ...
  /*
   *      Check if the specified name matches any known parameter.
   */
  for (index1 = 0; index1 < list->paramCnt; index1++) {
    ...
    for (offset = 0;; offset++) {
      ...
      if ((name[offset] == 0) && (token[offset] == 0)) {
        /*
      ...
      /*
       *              Parameter names are not case sensitive, so ignore
       *              any difference between upper/lower case
       */
      if (tolower(name[offset]) != tolower(token[offset])) {
        break;
      }
    }
  }
