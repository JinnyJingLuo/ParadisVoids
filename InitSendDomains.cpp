 ...
 *
 *-------------------------------------------------------------------------*/
static void PackInitialNodeData(InData_t *inData, int *nodeList, int nodeCount,
                                real8 **buf, int *bufEnts) {
  ...
  for (i = 0; i < nodeCount; i++) {
    ...
    for (armID = 0; armID < numNbrs; armID++) {
       ...
       */
      dataBuf[bufIndex++] = (real8)inData->node[nIndex].nbrTag[armID].domainID;
      dataBuf[bufIndex++] = (real8)inData->node[nIndex].nbrTag[armID].index;
      /*
       *              bx, by, bz
       */
      ...
      dataBuf[bufIndex++] = (real8)inData->node[nIndex].piChainID[armID];
    }
...
static void ExtendNodeKeys(Home_t *home, int newLength) {
  ...
  home->nodeKeys =
      (Node_t **)realloc(home->nodeKeys, newLength * sizeof(Node_t *));
...
static void UnpackInitialNodeData(Home_t *home, real8 *buf,
  ...
  for (i = 0; i < nodesInBuf; i++) {
    ...
    for (armID = 0; armID < numNbrs; armID++) {
      ...
      node->piChainID[armID] = (int)buf[bufIndex++];
    }
...
#ifdef PARALLEL
  home->inRequests =
      (MPI_Request *)malloc(home->numDomains * sizeof(MPI_Request));
  home->outRequests =
      (MPI_Request *)malloc(home->numDomains * sizeof(MPI_Request));
  home->inStatus = (MPI_Status *)malloc(home->numDomains * sizeof(MPI_Status));
  home->outStatus = (MPI_Status *)malloc(home->numDomains * sizeof(MPI_Status));
#endif
  ...
  if (listCounts != (int *)NULL) {
    ...
    for (domID = 0; domID < numDomains; domID++) {
      if (listCounts[domID] != 0) {
        ...
        PackInitialNodeData(inData, nodeLists[domID], listCounts[domID],
                            &sendBuf[sendIndex], &sendBufEnts[sendIndex]);
 ...
 */
  ...
  for (i = 0; i < numRecvBufs; i++) {
    MPI_Irecv(&recvBufEnts[i], 1, MPI_INT, MPI_ANY_SOURCE, MSG_INIT_LENS,
  ...
  for (i = 0; i < numSendBufs; i++) {
    if (i == selfSendIndex) {
      ...
      MPI_Isend(&sendBufEnts[i], 1, MPI_INT, sendBufDest[i], MSG_INIT_LENS,
   ...
   */
  if (selfSendIndex >= 0) {
    UnpackInitialNodeData(home, sendBuf[selfSendIndex], nextAvailableTag);
  }
 ...
 */
  ...
  for (i = 0; i < numRecvBufs; i++) {
    ...
    MPI_Irecv(recvBuf[i], recvBufEnts[i], MPI_DOUBLE,
              home->inStatus[i].MPI_SOURCE, MSG_INIT_NODES, MPI_COMM_WORLD,
              &home->inRequests[i]);
  }
  ...
  for (i = 0; i < numSendBufs; i++) {
    ...
    if (i == selfSendIndex) {
      ...
      MPI_Isend(sendBuf[i], sendBufEnts[i], MPI_DOUBLE, sendBufDest[i],
                MSG_INIT_NODES, MPI_COMM_WORLD, &home->outRequests[i]);
    }
  }
