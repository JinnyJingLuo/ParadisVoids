...
static void CommPackGhosts(Home_t *home) {
  ...
  for (idst = 0; idst < home->remoteDomainCount; idst++) {
    ...
    valCount = FLTS_PER_GHOST_NODE * totNodeCount +
               ...
               FLTS_PER_GHOST_CELL * remDom->numExpCells + EXTRA_GHOST_FLTS;
    ...
    for (iCell = 0; iCell < remDom->numExpCells; iCell++) {
      ...
      while (node) {
        ...
        /*
         *                  node's domainID is known implicitly, don't need to
         * send it
         */
        ...
        for (iNbr = 0; iNbr < nbrCount; iNbr++) {
           ...
           * pointer check!  On the BG/P system, this loop was having issues
           * (core dumps) accessing the nbrTag array.  It appears there may be a
           *                              compiler bug causing the problem.
           * Adding the 'if' test with the call to Fatal() before accessing
           * nbrTag seems to get us past the problem.  Sigh.
           */
          ...
          outBuf[bufIndex++] = (real8)node->piChainID[iNbr];
        }
        ...
        outBuf[bufIndex++] = node->dNx;
        outBuf[bufIndex++] = node->dNy;
        outBuf[bufIndex++] = node->dNz;

        outBuf[bufIndex++] = node->oldx;
        outBuf[bufIndex++] = node->oldy;
        outBuf[bufIndex++] = node->oldz;
      ...
      if (nodesPacked != cell->nodeCount) {
        Fatal("CommPackGhosts: dom %d, remDom %d, cell %d, "
              ...
              "nodesPacked value %d",
              home->myDomain, domainIdx, cellIdx, cell->nodeCount, nodesPacked);
      }
  ...
  for (isrc = 0; isrc < home->remoteDomainCount; isrc++) {
    ...
    for (iCell = 0; iCell < numExpCells; iCell++) {
      ...
      if (!cell) {
        ...
        Fatal("%s: received an unknown cell %d, (%d,%d,%d)", "CommUnpackGhosts",
              cellIdx, cX, cY, cZ);
      }
      ...
      for (inode = 0; inode < cellNodeCount; inode++) {
        ...
        for (iNbr = 0; iNbr < numNbrs; iNbr++) {
          ...
          node->nbrTag[iNbr].domainID = (int)inBuf[bufIndex++];
          node->nbrTag[iNbr].index = (int)inBuf[bufIndex++];
          ...
          node->piChainID[iNbr] = (int)inBuf[bufIndex++];
        }
        ...
        node->dNx = inBuf[bufIndex++];
        node->dNy = inBuf[bufIndex++];
        node->dNz = inBuf[bufIndex++];

        node->oldx = inBuf[bufIndex++];
        node->oldy = inBuf[bufIndex++];
        node->oldz = inBuf[bufIndex++];

        node->constraint = (int)inBuf[bufIndex++];
        node->flags = (int)inBuf[bufIndex++];

        /*
         *                  Register the node in the remote domain's nodeKeys
         * array, move node onto ghost queue, and add node to the cell's node
         * queue.
         */
...
void CommSendGhosts(Home_t *home) {
  ...
  for (isrc = 0; isrc < home->remoteDomainCount; isrc++) {
    ...
    MPI_Irecv(remDom->inBuf, valCount, MPI_DOUBLE, domainIdx, MSG_GHOST,
              MPI_COMM_WORLD, &home->inRequests[isrc]);
  ...
  for (idst = 0; idst < home->remoteDomainCount; idst++) {
    ...
    MPI_Isend(remDom->outBuf, valCount, MPI_DOUBLE, domainIdx, MSG_GHOST,
              MPI_COMM_WORLD, &home->outRequests[idst]);
  }
