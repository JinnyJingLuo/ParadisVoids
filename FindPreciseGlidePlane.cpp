 ...
 *
 *------------------------------------------------------------------------*/
static void FindBCCGlidePlane(real8 burg[3], real8 dir[3],
                              real8 glidePlane[3]) {
   ...
   */
  static real8 burgRef[13][3] = {{5.773503e-01, 5.773503e-01, 5.773503e-01},
   ...
   */
  MatrixMult((real8 *)burgRef, 13, 3, 3, burg, 1, 1, burgRefXburg, 1);
  ...
  MatrixMult(&glideRef[firstGlidePlaneIndex][0], numGlidePlanes, 3, 3,
             glidePlane, 1, 1, glideRefXglide, 1);
 ...
 *
 *------------------------------------------------------------------------*/
static void FindFCCGlidePlane(real8 burg[3], real8 dir[3],
                              real8 glidePlane[3]) {
  int burgIndex, firstGlidePlaneIndex, numGlidePlanes, glidePlaneIndex;
  ...
  static int planesPerBurg[21] = {3, 3, 3, 3, 3, 3, 4, 4, 4, 2, 2,
                                  2, 2, 2, 2, 2, 2, 2, 2, 2, 2};
  static int burgPlaneIndex[21] = {0,  3,  6,  9,  12, 15, 18, 22, 26, 30, 32,
                                   34, 36, 38, 40, 42, 44, 46, 48, 50, 52};
  static real8 glideRef[54][3] = {
      /* for burgRef[0][*] */
      {5.7735026919e-01, -5.7735026919e-01,
       5.7735026919e-01}, /* common glide plane */
      {-5.7735026919e-01, 5.7735026919e-01,
       5.7735026919e-01}, /* common glide plane */
   ...
   *      with this burgers vector.
   */
  MatrixMult((real8 *)burgRef, 21, 3, 3, burg, 1, 1, burgRefXburg, 1);
...
void FindPreciseGlidePlane(Home_t *home, real8 burgVecIn[3], real8 dirIn[3],
  ...
  if (home->param->useLabFrame) {
    Matrix33Vector3Multiply(home->rotMatrixInverse, burgVecIn, burgVecRotated);
    burgVec = burgVecRotated;
    Matrix33Vector3Multiply(home->rotMatrixInverse, dirIn, dirRotated);
    dir = dirRotated;
  }
   ...
   */
  if (fabs(glidePlane[X]) < 1.0e-06)
    glidePlane[X] = 0.0;
  if (fabs(glidePlane[Y]) < 1.0e-06)
    glidePlane[Y] = 0.0;
  if (fabs(glidePlane[Z]) < 1.0e-06)
    glidePlane[Z] = 0.0;
