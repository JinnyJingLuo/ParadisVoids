...
static void PackVelocity(Home_t *home) {
  ...
  for (i = 0; i < home->remoteDomainCount; i++) {
     ...
     */
    valCount = (nodesSent * VEL_FLTS_PER_NODE) + (armsSent * VEL_FLTS_PER_ARM) +
               VEL_FLTS_EXTRA;
    ...
    if (bufIndex > valCount) {
      Fatal("PackVelocity: packed %d values into %d element buffer", bufIndex,
            valCount);
    }
...
static void UnpackVelocity(Home_t *home) {
  ...
  for (i = 0; i < home->remoteDomainCount; i++) {
    ...
    for (j = 0; j < numNodes; j++) {
       ...
       * node are in exactly the same order as the arms of the original node, so
       * we have to look up the index for each arm before updating.
       */
      for (k = 0; k < numArms; k++) {
...
void CommSendVelocity(Home_t *home) {
  ...
  for (i = 0; i < home->remoteDomainCount; i++) {
    ...
    MPI_Irecv(&remDom->inBufLen, 1, MPI_INT, domainIdx, MSG_VELOCITY_LEN,
  ...
  for (i = 0; i < home->remoteDomainCount; i++) {
    ...
    MPI_Isend(&remDom->outBufLen, 1, MPI_INT, domainIdx, MSG_VELOCITY_LEN,
  ...
  for (i = 0; i < home->remoteDomainCount; i++) {
    ...
    MPI_Irecv(remDom->inBuf, valCount, MPI_DOUBLE, domainIdx, MSG_VELOCITY,
              MPI_COMM_WORLD, &home->inRequests[i]);
  }
  ...
  for (i = 0; i < home->remoteDomainCount; i++) {
    ...
    MPI_Isend(remDom->outBuf, valCount, MPI_DOUBLE, domainIdx, MSG_VELOCITY,
              MPI_COMM_WORLD, &home->outRequests[i]);
  }
