...
void SortNodesForCollision(Home_t *home) {
  ...
  probXmax = 0.5 * param->Dimensions[X];
  probYmax = 0.5 * param->Dimensions[Y];
  probZmax = 0.5 * param->Dimensions[Z];
   ...
   *      each cell, and the total number of cell2's in the minimum image
   * problem space
   */
  ...
  home->cell2QentArray = (C2Qent_t *)realloc(
      home->cell2QentArray, home->newNodeKeyPtr * sizeof(C2Qent_t));
  /*
   *      Loop through all nodes. Queue each node onto one of the cell2's. A
   * node may fall into more than one cell2, if the domain contains the whole
  ...
  for (i = 0; i < home->newNodeKeyPtr; i++) {
    ...
    GetInLocalCoordinates(home->param, x, y, z);
    GetNearestImage(param, centerX, centerY, centerZ, &x, &y, &z);
    ...
    /*
     *          Queue the node on the minimum cell2 that it falls into, but save
     * the ghost cell2 index in the node, since this is where it starts its
  ...
  for (cell2X = 0; cell2X < cell2nx; cell2X++) {
    ...
    if (cell2X >= numBodyCell2X)
      baseX = cell2X - numBodyCell2X;
    else
      baseX = cell2X;

    for (cell2Y = 0; cell2Y < cell2ny; cell2Y++) {

      if (cell2Y >= numBodyCell2Y)
        baseY = cell2Y - numBodyCell2Y;
      else
        baseY = cell2Y;

      for (cell2Z = 0; cell2Z < cell2nz; cell2Z++) {

        if (cell2Z >= numBodyCell2Z)
          baseZ = cell2Z - numBodyCell2Z;
        else
          baseZ = cell2Z;
