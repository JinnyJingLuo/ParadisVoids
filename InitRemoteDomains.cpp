...
void InitRemoteDomains(Home_t *home) {
  ...
  // allocate and initialize the array of all remote domains
  rDomMax = home->param->nXdoms * home->param->nYdoms * home->param->nZdoms;
  ...
  for (iCell = 0; iCell < home->nativeCellCount; iCell++) {
    ...
    natCell = home->cellKeys[natCellIdx];
    ...
    for (nCell = 0; nCell <= natCell->nbrCount; nCell++) {
      ...
      if (nCell == natCell->nbrCount) {
        ...
        nbrCell = home->cellKeys[nbrCellIdx];
      }
      /*
      ...
      for (iDom = 0; iDom < nbrCell->domCount; iDom++) {
        ...
        if (found == 0) {
          /*
           *                      If the export cell array has not yet been
           * allocated (or is too small) reallocate it with a small number of
           * spare entries.
           */
          if ((remDom->numExpCells % NUM_EXPCELL_INC) == 0) {
            newCount = remDom->numExpCells + NUM_EXPCELL_INC;
            remDom->expCells =
                (int *)realloc(remDom->expCells, newCount * sizeof(int));
          }
  ...
  for (iDom = 0; iDom < remDomCount; iDom++) {
    ...
    remDom = rDomKeys[domIdx];
    remDom->expCells =
        (int *)realloc(remDom->expCells, remDom->numExpCells * sizeof(int));
  }

  /*
   *      Move the remote domain structs into home
   */
  home->remoteDomains = (int *)realloc(remDomList, remDomCount * sizeof(int));
