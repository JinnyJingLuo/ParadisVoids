...
namespace FEMSystem
{
	FEMImplicitDynamicsSolidSolver::FEMImplicitDynamicsSolidSolver()
	{
		Initialize();
	}
	FEMImplicitDynamicsSolidSolver::FEMImplicitDynamicsSolidSolver(const FEMImplicitDynamicsSolidSolver& oSolver)
	{
		*this = oSolver;
	}
	FEMImplicitDynamicsSolidSolver::~FEMImplicitDynamicsSolidSolver()
	...
	FEMImplicitDynamicsSolidSolver& FEMImplicitDynamicsSolidSolver::operator=(const FEMImplicitDynamicsSolidSolver& oSolver)
	{
	...
	void FEMImplicitDynamicsSolidSolver::Reset()
	{ 		
		FEMSolver::Reset();
		Initialize();
	}
	void FEMImplicitDynamicsSolidSolver::Initialize()
	{
	...
	Vector FEMImplicitDynamicsSolidSolver::GetDisplacement(Point* poPoint,unsigned int& iStatus)
	{
	...
	Matrix FEMImplicitDynamicsSolidSolver::GetStress(Point* poPoint,unsigned int& iStatus)
	{
		Matrix oStress(3,3);
		vector<double> vdCoordinates;
		FEMElement* poElement = NULL;
	...
	void FEMImplicitDynamicsSolidSolver::GenerateMatrices(const unsigned int& iDOFCount)
	{		
		...
		for(i = 0; i < iSize ; i++)
		{
			...
			oTempMassMatrix = ((FEMSolidElement*)pvpoElements->at(i))->GetMassMatrix();
			dTemp = 0.0;
		...
		PartitionMatrices(oStiffness,oMass);
		PrintOnScreen("Done generating matrices");
	}
	void FEMImplicitDynamicsSolidSolver::PartitionMatrices(const SparseMatrix& oStiffnessMatrix,const SparseMatrix& oMassMatrix)
	{
		...
		for(i = 0; i < iSize1 ; i++)
		{
			for(j = 0; j < iSize1 ; j++)
			{
				...
				if(bMassFound || bStiffnessFound)
				{
					m_oDynamicMatrix11.Set(i + 1,j + 1,dStiffness + m_dA0*dMass);
				}
			}
		}
		for(i = 0; i < iSize1 ; i++)
		{
			for(j = 0; j < iSize2 ; j++)
			{
				...
				if(bMassFound || bStiffnessFound)
				{
					m_oDynamicMatrix12.Set(i + 1,j + 1,dStiffness + m_dA0*dMass);
				}
			}
		}
		for(i = 0; i < iSize2 ; i++)
		{
			for(j = 0; j < iSize1 ; j++)
			{
				...
				if(bMassFound || bStiffnessFound)
				{
					m_oDynamicMatrix21.Set(i + 1,j + 1,dStiffness + m_dA0*dMass);
				}
			}
		}
		for(i = 0; i < iSize2 ; i++)
		{
			for(j = 0; j < iSize2 ; j++)
			{
				...
				if(bMassFound || bStiffnessFound)
				{
					m_oDynamicMatrix22.Set(i + 1,j + 1,dStiffness + m_dA0*dMass);
				}
			}
		}
		// now since the dynamic matrix is built, generate its preconditioner
		m_oDynamicMatrix11.BuildPreconditioner();
		PrintOnScreen("Done partitioning matrices");
	}
	void FEMImplicitDynamicsSolidSolver::GenerateNodalStressesMatrix()
	{
	...
	void FEMImplicitDynamicsSolidSolver::InitializeMatrices(const double& dTimeStep)
	{
	...
	void FEMImplicitDynamicsSolidSolver::Solve(const double& dTime)
	{
		...
		Matrix oRHS1 = m_oPreviousDisplacements1*m_dA0 + m_oPreviousVelocities1*m_dA2 + m_oPreviousAccelerations1*m_dA3;
		Matrix oRHS2 = m_oPreviousDisplacements2*m_dA0 + m_oPreviousVelocities2*m_dA2 + m_oPreviousAccelerations2*m_dA3;
		Matrix oRHS = m_oForces1 - m_oDynamicMatrix12*m_oCurrentDisplacements2 + m_oMassMatrix11*oRHS1 + m_oMassMatrix12*oRHS2;
		m_oCurrentDisplacements1 = m_oDynamicMatrix11.SolveConjugateGradient(oRHS);
		// get the unknown forces
		m_oForces2 = m_oDynamicMatrix21*m_oCurrentDisplacements1 + m_oDynamicMatrix22*m_oCurrentDisplacements2;
		m_oForces2 = m_oForces2 - m_oMassMatrix21*oRHS1 - m_oMassMatrix22*oRHS2;
	...
	void FEMImplicitDynamicsSolidSolver::GenerateNodalStateVectors()
	{
	...
	void FEMImplicitDynamicsSolidSolver::UpdateNodalStateVectors()
	{
		vector<FEMNode*>* pvpoNodes = m_poDataStructure->GetNodes();
		unsigned iSize = (unsigned int)pvpoNodes->size();
		unsigned int i = 0;
	...
	void FEMImplicitDynamicsSolidSolver::WriteFEMSolutionToParaview(const unsigned int& iStep) const
	{
		char cWrite[512];
